            ...
            if self.lines[mid][0] <= parsed:
                try:
                    if self.lines[mid + 1][0] >= parsed:
                        try:
                            return unicode(self.lines[mid + 1][1]) + u":" + unicode(self.lines[mid + 1][2])
                        except:
                            return u""
                    else:
                        left = mid + 1
...
def parse(language, lineSource, skipWS = True, skipComments = None, packrat = False, lineCount = True):
    ...
    for line in lineSource:
        ...
        if type(line) is unicode:
            ...
            if sys.stdin.encoding:
                orig += codecs.decode(line, sys.stdin.encoding)
            else:
                orig += codecs.decode(line, "utf-8")
    ...
    try:
        ...
        raise SyntaxError(u"syntax error in " + file + u":" + unicode(l) + u": " + lineCont)
